...
def text(concrete_str):
    ...
    if concrete_str[1] == 3:
        ...
        cur = cur.replace("#N1", str(ss**power1))
        power2 = random.randint(1, 2)
        cur = cur.replace("#N2", str(ss ** power2))
        cur = cur.replace("#N3", str(ss))
        number1 = random.randint(300, 2018)
        cur = cur.replace("#N4", str(number1))
        number2 = random.randint(300, 2018)
        cur = cur.replace("#N5", str(number2))
        while True:  # генерим число 3, пока не добьемся выполнения условия
            power3 = random.randint(300, 1000)
            if power3 < number2 or power3 < number1:
                break
        cur = cur.replace("#N6", str(power3))

        answer = exc16_3(ss, number1, number2, power1, power2, power3)
        result = [cur, answer]
        return result
...
def exc16_3(ss, number1, number2, power1, power2, power3):
    tmp = (ss**power1)**number1 + (ss**power2)**number2 + ss**power3
    tmp = str(from10toX(tmp, ss))
    return str(tmp.count(str(0)))
